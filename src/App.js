import "./styles.css";
import { useEffect, useReducer, useState } from "react";
import { initialProducts } from "./initialProducts";
import { qtyReducer } from "./reducers/qtyReducer";

const headerItem = {
  borderTop: "1px solid #000",
  borderBottom: "1px solid #000",
  width: "20%"
};
const imgStyle = {
  height: "50px",
  width: "50px"
};
const changeQtyStyle = {
  fontWeight: "bold",
  cursor: "pointer"
};

export default function App() {
  const [productState, dispatch] = useReducer(qtyReducer, initialProducts);
  const [total, setTotal] = useState(0);
  function incrementQty(id) {
    console.log("increasing");
    console.log(id);
    dispatch({
      type: "increment_qty",
      id: id
    });
  }

  function decrementQty(id) {
    console.log("decreasing");
    // console.log(id);
    dispatch({
      type: "decrement_qty",
      id: id
    });
  }

  function handleDelete(productId) {
    console.log(productId);
    dispatch({
      type: "delete",
      id: productId
    });
  }

  useEffect(() => {
    const totalSum = productState.reduce(
      (accumulator, currentValue) =>
        accumulator + currentValue.qty * currentValue.price,
      0
    );
    setTotal(totalSum.toFixed(2));
  }, [productState]);

  return (
    <div className="App">
      <h1>Eccommerce</h1>
      <table>
        <thead>
          <tr>
            <th style={headerItem}>Product</th>
            <th style={headerItem}>Description</th>
            <th style={headerItem}>Quantity</th>
            <th style={headerItem}>Price</th>
            <th style={headerItem}>Remove</th>
          </tr>
        </thead>
        <tbody>
          {productState.map((item, index) => {
            // let tally = item.qty * item.price;

            return (
              <tr key={index}>
                <td>
                  <img style={imgStyle} src={item.imgSrc} alt="image" />
                </td>
                <td>{item.productName}</td>
                <td>
                  <span
                    style={changeQtyStyle}
                    onClick={() => decrementQty(item.productId)}
                  >
                    {" "}
                    &#60;{" "}
                  </span>
                  {item.qty}
                  <span
                    style={changeQtyStyle}
                    onClick={() => incrementQty(item.productId)}
                  >
                    {" "}
                    &#62;
                  </span>
                </td>
                <td>${item.price}</td>
                <td>
                  <p
                    onClick={() => {
                      handleDelete(index);
                    }}
                  >
                    X
                  </p>
                </td>
              </tr>
            );
          })}
        </tbody>
      </table>
      <div style={{ margin: "30px auto" }}>Total: {total}</div>
    </div>
  );
}
